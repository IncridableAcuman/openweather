...
const App: React.FC = () => {
  ...
  const getBackground = () => {
    if (!weather) return 'from-slate-900 to-indigo-950';
    const main = weather.weather[0].main.toLowerCase();
    if (main.includes('rain') || main.includes('drizzle')) return 'from-blue-800 to-slate-900';
    if (main.includes('snow')) return 'from-teal-700 to-blue-900';
    if (main.includes('clear')) return 'from-amber-500 to-blue-600';
    return 'from-sky-500 to-indigo-800'; // Bulutli yoki default
  };

  return (
    <div className={`min-h-screen bg-linear-to-br ${getBackground()} transition-all duration-1000 flex flex-col items-center justify-start p-6 font-sans overflow-x-hidden`}>
